<script lang="ts">
	...
	import { X, HeadphoneOff, Headphones, Volume2, VolumeOff, Trash2 } from '@lucide/svelte';
	...
	function updatePlayhead(event) {
		if (mouseDown) {
			playhead.pos = (event.clientX - trackAreaStartX) / viewScale;
		}
	}
...
<ContextMenu.Root>
	<ContextMenu.Trigger>
		...
		>
			<div bind:clientWidth={trackAreaStartXValue} class="flex">
				<span
					class="flex h-full select-none flex-col items-center justify-between bg-gray-300 font-mono text-gray-700 dark:bg-gray-700 dark:text-gray-300"
				>
					<Button variant="ghost" class="w-full rounded-none" onclick={() => deleteTrack(index)}
					...
					<div class="flex w-full max-w-full flex-row gap-2 p-2">
						<span class="flex gap-1 text-sm text-zinc-400">
							<div class="flex w-16 justify-end">
								<NumericInput bind:value={track.gain} min={-100} max={6} step={1} plus={true} />
							</div>
							<span class="select-none">dB</span>
						</span>
						<Slider
			...
			<div
				class="relative flex w-full border-t-2 border-zinc-900 bg-neutral-950 active:cursor-col-resize"
				class:border-b-2={index === trackCount - 1}
	...
	<ContextMenu.Content>
		...
		<ContextMenu.Item onclick={() => soloTrack(index)}
			>{#if track.soloMute.includes('solo')}<HeadphoneOff />Unsolo{:else}<Headphones
				/>Solo{/if}</ContextMenu.Item
		>
		...
		>
		<ContextMenu.Item onclick={() => deleteTrack(index)}><Trash2 />Delete</ContextMenu.Item>
	</ContextMenu.Content>
</ContextMenu.Root>
