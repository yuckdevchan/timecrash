<script lang="ts">
	...
	import { FilePlay, Scaling, Import, Plus, Pencil, Trash2, ListEnd } from '@lucide/svelte';
...
<div class="mt-2 flex w-full bg-zinc-900">
	<Button
...
<section class="p-2">
	<div class="flex items-center justify-between">
		<h2 class="select-none pl-3">Media Pool</h2>
		<div class="flex items-center gap-3">
			<Scaling size={20} />
			<Slider type="single" min={0} max={6} bind:value={mediaPoolDensity} class="w-40"></Slider>
		</div>
	</div>
	<div>
		<div class="flex select-none items-center gap-1 p-2 text-zinc-400">
			<div></div>
			...
			{#each mediaPool as file, index (file.id)}
				...
				<ContextMenu.Root>
					<ContextMenu.Trigger>
						<li
							draggable="true"
							class="flex select-none items-center gap-1 p-{mediaPoolDensity} hover:bg-zinc-800"
						...
						>
							...
							<div class="flex w-full items-center justify-between">
								...
								<div class="flex items-center gap-8">
									...
									<span class="text-zinc-600">{Math.round(file.size / 1000000, 3)} MB</span>
									<span class="text-zinc-600">{formatUnixTimestamp(file.lastModified)}</span>
								</div>
							</div>
						</li>
					</ContextMenu.Trigger>
					<ContextMenu.Content>
						<ContextMenu.Sub>
							<ContextMenu.SubTrigger><ListEnd />Add to track</ContextMenu.SubTrigger>
							<ContextMenu.SubContent>
								{#each tracks as track, index (track.id)}
									...
									>
										<span class="flex items-center font-mono text-zinc-600">{index + 1}</span
										>{track.name}
									</ContextMenu.Item>
								{/each}
								<ContextMenu.Item
